...
export default function WebtoonsPage() {
  ...
  useEffect(() => {
    fetchPublishedWebtoons();
  }, []);

  const fetchPublishedWebtoons = async () => {
    try {
      setLoading(true);
      ...
      if (data.success) {
        setWebtoonList(data.works);
        
        // Extract all unique genres from webtoons
        const allGenres = new Set<string>();
        data.works.forEach((webtoon: Webtoon) => {
          if (webtoon.genres && Array.isArray(webtoon.genres)) {
            webtoon.genres.forEach(genre => allGenres.add(genre));
          }
        });
  ...
  const toggleFavorite = (e: React.MouseEvent, webtoon: Webtoon) => {
    e.preventDefault();
    e.stopPropagation();
    
    if (isFavorite(webtoon.id, 'webtoon')) {
      removeFromFavorites(webtoon.id, 'webtoon');
    } else {
      addToFavorites({
        id: webtoon.id,
        title: webtoon.title,
        ...
        latest_chapter_date: webtoon.latest_chapter_date
      });
    }
  };
  ...
  const navigateToAllWebtoons = () => {
    router.push('/webtoons/all');
  };

  const navigateToGenrePage = (genre: string) => {
    router.push(`/webtoons/genre/${encodeURIComponent(genre)}`);
  };

  // Horizontal Scroll Webtoon Row Component
  const WebtoonRow = ({ title, webtoonList, onViewAll }: { title: string, webtoonList: Webtoon[], onViewAll?: () => void }) => {
    if (webtoonList.length === 0) return null;
              ...
              {webtoonList.map((webtoon) => (
                ...
                >
                  ...
                  <div className="relative aspect-[3/4] overflow-hidden">
                    {webtoon.cover_url && webtoon.cover_url !== '/placeholder-cover.jpg' ? (
                      <img 
                    ...
                    {/* Favorite Button */}
                    <button
                      onClick={(e) => toggleFavorite(e, webtoon)}
                      className={`absolute top-2 right-2 p-1.5 rounded-full transition-all ${
                        isFavorite(webtoon.id, 'webtoon')
                          ? 'bg-red-500 text-white'
                          : 'bg-black/50 text-white opacity-0 group-hover:opacity-100 hover:bg-red-500'
                      }`}
                    >
                      <Heart className={`w-3 h-3 ${isFavorite(webtoon.id, 'webtoon') ? 'fill-current' : ''}`} />
                    </button>
                  </div>
                  
                  {/* Webtoon Info */}
                  <div className="p-2">
                    <h3 className="text-xs font-semibold text-gray-900 dark:text-white line-clamp-1 mb-1.5" title={webtoon.title}>
                      {webtoon.title}
                    </h3>
    ...
    return (
      <div className="min-h-screen bg-gray-50 dark:bg-black transition-colors duration-300 pt-16 font-sans">
        <div className="container mx-auto px-4 py-8 text-center">
          ...
          <p className="text-gray-600 dark:text-gray-400">Loading webtoons...</p>
        </div>
      </div>
    );
  }
        ...
        <div className="pt-4 md:pt-12 pb-8">
          ...
          {webtoonList.length === 0 ? (
            <div className="text-center py-12 bg-white dark:bg-gray-900 rounded-xl border border-gray-300 dark:border-gray-800">
              ...
              <h3 className="text-xl font-bold text-gray-900 dark:text-white mb-2">No Webtoons Available</h3>
            ...
            <>
              ...
              <WebtoonRow 
                ...
                onViewAll={navigateToAllWebtoons}
              />

              {/* GENRE ROWS */}
              {genres.map(genre => {
                const genreWebtoons = webtoonList.filter(webtoon => 
                  webtoon.genres && webtoon.genres.includes(genre)
                );
                
                if (genreWebtoons.length === 0) return null;
                
                return (
                  <WebtoonRow 
                    key={genre}
                    title={`${genre} Webtoons`}
                    webtoonList={genreWebtoons}
